...
CREATE OR REPLACE FUNCTION is_fruit(product_type) RETURNS boolean AS $_$
  SELECT $1 BETWEEN 1000 AND 1999;
$_$ LANGUAGE sql IMMUTABLE;

CREATE OR REPLACE FUNCTION is_apple(product_type) RETURNS boolean AS $_$
  SELECT $1 BETWEEN 1100 AND 1199;
$_$ LANGUAGE sql IMMUTABLE;

CREATE OR REPLACE FUNCTION is_banana(product_type) RETURNS boolean AS $_$
  SELECT $1 BETWEEN 1200 AND 1299;
$_$ LANGUAGE sql IMMUTABLE;

CREATE OR REPLACE FUNCTION is_berry(product_type) RETURNS boolean AS $_$
  SELECT $1 BETWEEN 1300 AND 1399;
$_$ LANGUAGE sql IMMUTABLE;

CREATE OR REPLACE FUNCTION is_trouble_supplier(integer) RETURNS boolean AS $_$
  SELECT $1 IN (32, 101);
$_$ LANGUAGE sql IMMUTABLE;

CREATE OR REPLACE FUNCTION is_premium_supplier(integer) RETURNS boolean AS $_$
  SELECT $1 IN (204, 219);
$_$ LANGUAGE sql IMMUTABLE;

CREATE TEMP TABLE produce (
...
\copy produce FROM 'produce.csv' WITH CSV HEADER QUOTE '"';

WITH
produce_markup (supplier_id, markup, price, max_sell_date, description, item_no) AS (
    SELECT 
        supplier_id,
        CASE 
...
priced_produce (price, max_sell_date, description) AS (
    SELECT
        CASE
...
SELECT 'R'||substr(to_char(price, '99990.00'), 2)||to_char(max_sell_date, 'YYYY/MM/DD')||substr(description,1 ,31) FROM priced_produce;
